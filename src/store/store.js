  ...
  actions: {
    getCompanyList(context, params) {
      return new Promise((resolve, reject) => {
        if (context.getters.companyList.length <= 1 || params.type) {
          axios
            .get("/company/list", {
              params: {
            ...
            .then(data => {
              context.commit("getCompanyList", data.data.companyList);
              resolve();
            });
        } else {
          resolve();
    ...
    getAlertList(context, params) {
      return new Promise((resolve, reject) => {
        if (
          context.getters.alertList.length <= 1 ||
          params.type == "force" ||
          ...
          if (context.getters.userInfo.role <= 1) {
            ...
            context.getters.userInfo.role >= 1 &&
            context.getters.userInfo.role <= 3
          ) {
            axios
              .get("/alert/list", {
                params: {
    ...
    getOrganizeList(context, params) {
      return new Promise((resolve, reject) => {
        if (
          context.getters.organizeList.length <= 1 ||
          params.type == "force"
        ) {
          axios
            .get("/organize/list", {
